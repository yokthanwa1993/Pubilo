...
// Nav Icons (same style as dubbing app - outline / filled)
...
const IconQuote = ({ filled }: { filled: boolean }) => filled ? (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
...
const IconLog = ({ filled }: { filled: boolean }) => filled ? (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
...
const IconPages = ({ filled }: { filled: boolean }) => filled ? (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
...
const IconEarnings = ({ filled }: { filled: boolean }) => filled ? (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
...
export default function App() {
  ...
  const NavIcon = ({ id, active }: { id: Tab; active: boolean }) => {
    const p = { filled: active }
    switch (id) {
      case 'home': return <IconHome {...p} />
      case 'quotes': return <IconQuote {...p} />
      case 'logs': return <IconLog {...p} />
      case 'pages': return <IconPages {...p} />
      case 'earnings': return <IconEarnings {...p} />
    }
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100dvh', background: 'var(--bg)', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', WebkitOverflowScrolling: 'touch' }}>
        {tab === 'home' && <HomeTab pages={pages} loadingPages={loadingPages} onRefreshPages={loadPages} />}
        {tab === 'quotes' && <QuotesTab />}
        {tab === 'logs' && <LogsTab pages={pages} />}
        {tab === 'pages' && <PagesTab pages={pages} onRefresh={loadPages} />}
        {tab === 'earnings' && <EarningsTab pages={pages} />}
      </div>

      {/* Bottom Nav — exact same style as dubbing app */}
      <div style={{
        position: 'fixed',
        bottom: 0,
        left: 0,
        ...
        <div style={{ display: 'flex', paddingTop: 8, paddingBottom: 4 }}>
          {TABS.map(t => {
            ...
            return (
              <button
                ...
                style={{
                  ...
                  cursor: 'pointer',
                  color: active ? 'var(--accent)' : 'rgba(0,0,0,0.35)',
                  position: 'relative',
                  WebkitTapHighlightColor: 'transparent',
                  transition: 'color 0.2s',
                }}
              >
                {/* Active top bar indicator (same as dubbing app) */}
                {active && (
                  <div style={{
                    position: 'absolute',
                    top: 0,
